...
class Home extends React.Component {
    ...
    return (
      <section className="l-content">
        <div className="m-form">
          <h1 className="e-title">Secret Santa</h1>
          <p>Welcome to Laura's awesome secret santa app!</p>
          <p>Add your details below and we'll email your results.</p>
          <div className="e-formfield">
  ...
  submit() {
    var fakeData = [
      {
        ...
        address:
          'Flat 209, 3 Cambridge Street, Chorlton Mill, Manchester M15BY',
        cantHave: [1]
      },
      {
        id: 1,
    ...
    for (var i = 0; i < restrictedFirst.length; i++) {
      ...
      var cantHaves = thisGuy.cantHave
        .concat([thisGuy.id])
        .concat(dataToSend.map(x => x.ss_id));
      //console.log("["+ thisGuy.name  +" can't have]", cantHaves);

      var canHave = restrictedFirst
      ...
      if (canHave.length === 0) {
        ...
        var randomAppropriatePerson = restrictedFirst.find(function(person) {
          ...
          if (theirData) {
            ...
            return (
              person.cantHave.indexOf(thisGuy.id) === -1 &&
              thisGuy.cantHave.indexOf(theirPerson.id) === -1
            );
          }
        ...
        console.log(
          '[amendment for ' + sendingData.to_name + ']',
          thisGuy.name
        );
